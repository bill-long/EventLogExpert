...
namespace EventLogExpert
{
    public class RegistryProvider
    {
        ...
        public IEnumerable<string> GetMessageFilesForLegacyProvider(string providerName)
        {
            ...
            if (eventLogKey == null) throw new OpenEventLogRegistryKeyFailedException(ComputerName);

            foreach (var logSubKeyName in eventLogKey.GetSubKeyNames())
            {
                // Skip Security since it requires elevation
                if (logSubKeyName == "Security") continue;
                ...
                if (eventMessageFilePath == null) continue;
        ...
        private IEnumerable<string> GetExpandedFilePaths(IEnumerable<string> paths)
        {
            ...
            paths = paths.Select(p =>
            {
                ...
                if (match.Success) newPath = $"\\\\{ComputerName}\\{match.Value[0]}${newPath.Substring(2)}";
